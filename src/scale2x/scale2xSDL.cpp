...
void scale2x(SDL_Surface *src, SDL_Surface *dst)
{
     if(SDL_LockSurface(src) < 0)
        Log("scale2x:Was not able to lock work surface");

    if(SDL_LockSurface(dst) < 0)
        Log("scale2x:Was not able to lock screen surface");
...
void simple2x(SDL_Surface *src, SDL_Surface *dst)
{
     if(SDL_LockSurface(src) < 0)
        Log("scale2x:Was not able to lock work surface");
	...
	switch (src->format->BytesPerPixel)
	{
	    ...
	    {
	        for(y = 0; y < height; ++y) {
                Uint16 *xsrc = (Uint16*)(srcpix + y * src->pitch);
                Uint16 *xdst1 = (Uint16*)(dstpix + y*2 * dst->pitch);
                Uint16 *xdst2 = (Uint16*)(dstpix + (y*2 + 1) * dst->pitch);
                for(x = 0; x < width; ++x) {
                    *xdst1++ = *xsrc;
                    *xdst1++ = *xsrc;
                    *xdst2++ = *xsrc;
